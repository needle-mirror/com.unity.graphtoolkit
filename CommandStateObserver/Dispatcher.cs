...
namespace Unity.GraphToolkit.CSO
{
    ...
    /// </summary>
    [UnityRestricted]
    internal class Dispatcher : ICommandTarget
    {
        /// <summary>
        /// The mapping of command types to command handlers.
        /// </summary>
        protected readonly Dictionary<Type, ICommandHandlerFunctor> m_CommandHandlers = new();

        /// <summary>
        /// The list of actions that needs to be invoked before executing a command.
        /// </summary>
        protected readonly List<Action<ICommand>> m_CommandPreDispatchCallbacks = new();
        ...
        public void RegisterCommandHandler(ICommandHandlerFunctor commandHandlerFunctor)
        {
            if (!IsDispatching)
            {
                var commandType = commandHandlerFunctor.CommandType;
        ...
        {
            return m_CommandHandlers.GetValueOrDefault(commandType);
        }
