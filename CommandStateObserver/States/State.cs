...
namespace Unity.GraphToolkit.CSO
{
    ...
    /// </summary>
    [UnityRestricted]
    internal class State : IState, IDisposable
    {
        bool m_Disposed;
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                for (var i = AllStateComponents.Count - 1; i >= 0; i--)
                {
                    var stateComponent = AllStateComponents[i];
                    RemoveStateComponent(stateComponent);
