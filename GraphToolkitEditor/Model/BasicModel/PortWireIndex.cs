...
namespace Unity.GraphToolkit.Editor
{
    ...
    /// </remarks>
    class PortWireIndex<TWire> where TWire : class, IPortWireIndexModel
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="PortWireIndex{TWire}"/> class to index <paramref name="wireModels"/>.
        /// </summary>
        public PortWireIndex(IReadOnlyList<TWire> wireModels)
        {
        ...
        public void PortUniqueNameChanged(PortModel portModel, string oldName, string newName)
        {
            ...
            if (!m_WiresByPort.Remove(key, out var wireList))
            {
                return;
            }
        ...
        public void WireReordered(TWire wireModel, ReorderType reorderType)
        {
            ...
            {
                throw new IndexOutOfRangeException($"{wireModel} not part of the {typeof(PortWireIndex<>).Name}.");
            }
        }

        internal class TestAccess<TTestWire> where TTestWire : class, IPortWireIndexModel
        {
            readonly PortWireIndex<TTestWire> m_PortWireIndex;

            public TestAccess(PortWireIndex<TTestWire> portWireIndex)
            {
                m_PortWireIndex = portWireIndex;
            }

            public bool TryGetWiresForPort(Hash128 guid, string uniqueName, PortDirection direction,
                out List<TTestWire> wireList)
            {
                if (m_PortWireIndex.m_IsDirty)
                    m_PortWireIndex.Reindex();

                return m_PortWireIndex.m_WiresByPort.TryGetValue((guid, uniqueName, direction), out wireList);
            }
        }
    }
}
