...
{
    static class YamlParsingHelper
    {
        ...
        static bool TryParseVector2Format1(string data, string field, int startingIndex, out Vector2 parsedVector2)
        {
            ...
            var posX = float.Parse(data.Substring(openingParenthesisIndex + 1, spaceIndex - openingParenthesisIndex), CultureInfo.InvariantCulture.NumberFormat);
            var posY = float.Parse(data.Substring(spaceIndex + 1, closingParenthesisIndex - spaceIndex - 1), CultureInfo.InvariantCulture.NumberFormat);
        ...
        static bool TryParseVector2Format2(string data, string field, int startingIndex, out Vector2 parsedVector2)
        {
            ...
            if (TryParseLine(field, data, fieldIndex, out var valueStr))
            {
                ...
                var xValue = float.Parse(valueStr.Substring(startXValueIndex, endXValueIndex - startXValueIndex), CultureInfo.InvariantCulture.NumberFormat);
                var yValue = float.Parse(valueStr.Substring(startYValueIndex, endYValueIndex - startYValueIndex), CultureInfo.InvariantCulture.NumberFormat);
