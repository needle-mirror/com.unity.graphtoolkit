...
namespace Unity.GraphToolkit.Editor
{
    ...
    class SampleDependencyImporter : IPackageManagerExtension
    {
        ...
        const string k_PackageName = "com.unity.graphtoolkit";
        ...
        /// It loads the GraphToolkit package info and configuration, when GraphToolkit is selected.
        /// </summary>
        public void OnPackageSelectionChange(PackageManager.PackageInfo packageInfo)
        {
            bool packageInfoValid = packageInfo != null && packageInfo.name.StartsWith(k_PackageName);
            if (m_PackageInfo == null && packageInfoValid)
            {
                ...
                foreach (var sample in m_Samples)
                {
                    if (assetPath.EndsWith(sample.displayName))
                    {
                        ...
                        if (sampleEntry != null)
                        {
                            ...
                            if (!string.IsNullOrEmpty(sampleEntry.Message))
                                Debug.Log(sampleEntry.Message);
        ...
        static bool PackageManagerUIOpen() => EditorBridge.PackageManagerUIOpen();
