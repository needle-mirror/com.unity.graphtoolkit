...
namespace Unity.GraphToolkit.Editor
{
    ...
    /// </summary>
    [UnityRestricted]
    internal static class ModelViewFactory
    {
        /// <summary>
        ...
        /// <param name="parentView">An optional <see cref="ChildView"/> parent.</param>
        /// <typeparam name="T">The type of the returned object.</typeparam>
        /// <returns>An instance of <see cref="ModelView"/> that display <paramref name="model"/>.</returns>
        public static T CreateUI<T>(RootView view, Model model, IViewContext context, ChildView parentView = null) where T : ModelView
        {
            if (view == null)
            {
            ...
            if (ext != null)
            {
                var nodeBuilder = new ElementBuilder { View = view, Context = context, ParentView = parentView};
                try
                {
