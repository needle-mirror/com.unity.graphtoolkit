...
namespace Unity.GraphToolkit.Editor
{
    ...
    {
        /// <summary>
        /// Default scale that should be computed when scroll wheel offset is at zero.
        /// </summary>
        public static readonly float DefaultReferenceScale = 1.0f;

        /// <summary>
        /// Default minimum scale authorized by the <see cref="ContentZoomer"/>.
        /// </summary>
        public static readonly float DefaultMinScale = 0.1f;

        /// <summary>
        /// Default maximum scale authorized by the <see cref="ContentZoomer"/>.
        /// </summary>
        public static readonly float DefaultMaxScale = 4.0f;

        /// <summary>
        /// Default step between stepswhen using the scroll wheel.
        /// </summary>
        ...
        /// <summary>
        /// The current minimal scale allowed by the manipulator.
        /// </summary>
        public float minScale { get; set; } = DefaultMinScale;

        /// <summary>
        /// The current maximal scale allowed by the manipulator.
        /// </summary>
        ...
        protected void OnWheel(WheelEvent evt)
        {
            ...
            // Apply the new zoom.
            float zoom = CalculateNewZoom(scale.y, -evt.delta.y, scaleStep * (evt.delta.magnitude / WheelEvent.scrollDeltaPerTick),
                referenceScale, minScale, maxScale);
            scale.x = zoom;
            scale.y = zoom;
            scale.z = 1;

            position -= Vector3.Scale(new Vector3(x, y, 0), scale);
            position.x = graphView.RoundToPanelPixelSize(position.x);
            position.y = graphView.RoundToPanelPixelSize(position.y);
