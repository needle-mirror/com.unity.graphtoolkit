    ...
    internal class WireManipulator : MouseManipulator
    {
        ...
        protected void OnMouseMove(MouseMoveEvent evt)
        {
            ...
            if (m_Wire == null)
            {
                ...
                EditorGUIUtilityBridge.SetCursor(MouseIsNearPorts(inputPortUI, outputPortUI, evt.mousePosition, out _, out _) ? MouseCursor.Link : MouseCursor.Arrow);
                evt.StopPropagation();
            ...
            if (!alreadyDetached)
            {
                ...
                m_ConnectedWireDragHelper.CreateWireCandidate();
                m_ConnectedWireDragHelper.WireCandidateModel.ToWorldPoint = evt.mousePosition;
                ...
                if (m_ConnectedWireDragHelper.HandleMouseDown(m_LastMouseDownEvent, compatiblePort =>
                {
                    ...
                    if (needsSetup)
                    {
                        m_AdditionalWireDragHelpers = GetAdditionalWireDragHelpers(m_DetachedPort, m_Wire, view, evt);
                        needsSetup = false;
                    }
            ...
            foreach (var wire in connectedWires)
            {
                ...
                if (wireUI != null && wireUI != targetWire && wireUI.IsSelected())
                {
                    ...
                    if (wireDragHelper != null)
                    {
                        ...
                        wireDragHelper.WireCandidateModel.ToWorldPoint = evt.mousePosition;
