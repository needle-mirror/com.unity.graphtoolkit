...
namespace Unity.GraphToolkit.Editor
{
    class BlockDragInfos
    {
        ...
        ContextNodeView HoveredContext { get; set; }
        ModelView DraggedBlock { get; set; }
        public ModelView DraggedBlockContext { get; }
        ...
        public BlockDragInfos(BlockNodeView draggedBlock)
        {
            GraphView = draggedBlock.GraphView;
            DraggedBlock = draggedBlock;
            DraggedBlockContext = draggedBlock.BlockNodeModel.ContextNodeModel.GetView<ContextNodeView>(GraphView);
        }
                ...
                if ((e.mousePosition - StartDrag).sqrMagnitude > k_DragThresholdSquare)
                {
                    ...
                    if (m_Duplicate)
                    {
                        foreach (var block in SelectedBlockModels)
                        {
                            ...
                            if (newBlockUI is GraphElement ge)
                                ge.SetLevelOfDetail(1, GraphViewZoomMode.Normal, GraphViewZoomMode.Unknown);
                ...
                using (var updater = GraphView.GraphViewModel.GraphModelState.UpdateScope)
                {
                    if (SelectedBlocks != null)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                var context =
                                    ((BlockNodeModel)block.Model).ContextNodeModel.GetView<ContextNodeView>(GraphView);
                                if (context == null)
                                    continue;
