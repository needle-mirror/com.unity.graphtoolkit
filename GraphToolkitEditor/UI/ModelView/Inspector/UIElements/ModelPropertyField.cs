...
namespace Unity.GraphToolkit.Editor
{
    ...
    [UnityRestricted]
    internal class ModelPropertyField<TValue, TCommand> : ModelPropertyField<TValue>
    ...
    internal class ModelPropertyField<TValue> : CustomizableModelPropertyField
    {
        ...
        public override void UpdateDisplayedValue()
        {
            if (m_ValueGetter != null)
            {
                ...
                else
                {
                    ...
                    else if (m_CustomPropertyDrawerAdapter != null)
                    {
                        if (m_CustomPropertyDrawerAdapter.UpdateDisplayedValue(newValue))
                            return;
                    }
                    else
                    {
                        switch (Field)
                        {
                            ...
                            case BaseField<Enum> baseField:
                            {
                                ...
                                {
                                    baseField.SetValueWithoutNotify(bValue ? Bool.True : Bool.False);
                                    return;
                                }
        ...
        void CreateFieldFromProperty(string propertyName, string fieldTooltip)
        {
            if (m_CustomFieldBuilder == null && m_CustomPropertyDrawerAdapter == null)
            {
                ...
                if (m_CustomFieldBuilder == null)
                {
                    m_CustomPropertyDrawerAdapter = CustomPropertyDrawerAdapter.Create(propertyName, CommandTarget, Models);
                }
            }

            if (m_CustomFieldBuilder != null)
            {
                var(label, field) = m_CustomFieldBuilder.Build(CommandTarget, Label, fieldTooltip, Models, propertyName);
                Setup(label, field, fieldTooltip);
            }

            if (m_CustomPropertyDrawerAdapter != null)
            {
                var field = m_CustomPropertyDrawerAdapter.Build(Label);
                Setup(null, field, fieldTooltip);
            }
