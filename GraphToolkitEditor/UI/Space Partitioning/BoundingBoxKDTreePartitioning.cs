...
namespace Unity.GraphToolkit.Editor
{
        ...
        where TElementKey : IEquatable<TElementKey>
    {
        ...
        /// </summary>
        [UnityRestricted]
        internal enum NodeIterationContinuation
        {
            ContinueAll,
            ContinueLeftOnly,
            ContinueRightOnly,
        ...
        /// </summary>
        [UnityRestricted]
        internal class NodeVisitContext
        {
            /// <summary>
            /// The current <see cref="NodeIterationContinuation"/>.
            /// </summary>
            public NodeIterationContinuation Continuation = NodeIterationContinuation.ContinueAll;
        }
        ...
        internal class KdTreeNode
        {
            ...
            public KdTreeNode(TElementKey elementKey, Axis axis, float axisValue, Rect boundingBox)
            {
        ...
        {
            public readonly KdTreeNode Node;
            public readonly int PivotIndex;
            public readonly int SubTreeStart;
            public readonly int SubTreeLength;

            public NodeBuildInfo(KdTreeNode node, int pivotIndex, int subTreeStart, int subTreeLength)
            {
                Node = node;
        ...
        /// </summary>
        protected KdTreeNode m_RootNode;
        ...
        // For testing only.
        public KdTreeNode Root => m_RootNode;
        ...
        protected bool RemoveElements(ICollection<TElementKey> elementKeys, ProcessingContext context)
        {
            ...
            var nodesToRemove = new List<KdTreeNode>(capacity: elementKeys.Count);

            // Iterating over all nodes to see which ones are to be removed seems to be
            // faster than having a map of TElementKey <--> KdTreeNode and checking which ones are in the map.
            IterateNodes(m_RootNode, (node, ctx) =>
            {
                if (ContainsKey(elementKeys, node.ElementKey))
                    nodesToRemove.Add(node);
                ctx.Continuation = NodeIterationContinuation.ContinueAll;
            });
        ...
        protected bool RemoveElements(IReadOnlyCollection<BoundingBoxElement> elements, ProcessingContext context)
        {
            ...
            var nodesToRemove = new List<KdTreeNode>(capacity: elements.Count);
        ...
        /// <param name="context">The processing context.</param>
        /// <returns>True if the operation completed, false if it stopped because it reached the threshold.</returns>
        protected bool RemoveNodes(List<KdTreeNode> nodesToRemove, ProcessingContext context)
        {
                    ...
                    switch (currentAxis)
                    {
                        ...
                        case Axis.YMin:
                            if (currentAxisValue > rectAxisValue)
                                context.Continuation = NodeIterationContinuation.ContinueLeftOnly;
                            break;
                        case Axis.XMax:
                        case Axis.YMax:
                            if (currentAxisValue < rectAxisValue)
                                context.Continuation = NodeIterationContinuation.ContinueRightOnly;
                            break;
                        default:
        ...
        protected void AddElement(in BoundingBoxElement element)
        {
            if (m_RootNode == null)
            {
                m_RootNode = CreateNode(element, Axis.XMin);
                return;
            }
        ...
        protected void AddElement(in BoundingBoxElement element, KdTreeNode rootNode)
        {
        ...
        protected bool RemoveNode(KdTreeNode node, ProcessingContext context)
        {
            ...
            var nodeStack = new Stack<KdTreeNode>();
        ...
        protected KdTreeNode FindReplacementNode(KdTreeNode node)
        {
        ...
        protected virtual KdTreeNode BuildTree(Span<BoundingBoxElement> elements, Axis currentAxis)
        {
        ...
        protected void RemoveAndRebuild(ICollection<TElementKey> elementKeys)
        {
            ...
            m_RootNode = BuildTree(newElements, Axis.XMin);
            base.AddOrUpdateElements(newElements);
        }

        class UpdateAndRebuildVisitContext : NodeVisitContext
        {
            public int Index;
        }
        ...
        /// <returns></returns>
        protected static KdTreeNode CreateNode(in BoundingBoxElement element, Axis currentAxis)
        {
            return new KdTreeNode(element.Key, currentAxis, GetBoundingBoxValueByAxis(element, currentAxis), element.BoundingBox);
        }
        ...
        protected static Axis GetNextAxis(Axis axis)
        {
            switch (axis)
            {
                case Axis.XMin:
        ...
        /// <exception cref="ArgumentOutOfRangeException">Throws an exception if the current axis has an invalid value.</exception>
        protected static Axis GetOppositeAxis(Axis axis)
        {
            switch (axis)
            {
                case Axis.XMin:
        ...
        protected static float GetBoundingBoxValueByAxis(in Rect boundingBox, Axis axis)
        {
            switch (axis)
            {
                case Axis.XMin:
        ...
        /// <exception cref="ArgumentOutOfRangeException">Throws an exception if the axis has an invalid value.</exception>
        protected static float GetVector2ValueByAxis(in Vector2 vector2, Axis axis)
        {
        ...
        /// <param name="axis">The splitting axis.</param>
        /// <returns>The bounding box value for the axis.</returns>
        protected static float GetBoundingBoxValueByAxis(in BoundingBoxElement bbe, Axis axis)
        {
        ...
        /// <param name="axis">The splitting axis.</param>
        /// <returns>The bounding box value for the axis.</returns>
        protected static float GetBoundingBoxValueByAxis(KdTreeNode node, Axis axis)
        {
            return GetBoundingBoxValueByAxis(node.BoundingBox, axis);
        }

        /// <summary>
        /// Iterates over the nodes with a default <see cref="NodeVisitContext"/>.
        /// </summary>
        /// <param name="rootNode">The starting node.</param>
        /// <param name="visitor">The visitor callback.</param>
        protected void IterateNodes(KdTreeNode rootNode, Action<KdTreeNode, NodeVisitContext> visitor)
        {
            var context = new NodeVisitContext();
        ...
        /// <param name="visitor">The visitor callback.</param>
        protected void IterateNodes<TContext>(KdTreeNode rootNode, TContext nodeVisitContext, Action<KdTreeNode, TContext> visitor)
            where TContext : NodeVisitContext
        {
            if (rootNode == null)
                return;

            // Estimate the depth of the tree using Base2 Log.
            Stack<KdTreeNode> nodesToVist = new Stack<KdTreeNode>((int)Math.Log(Count, 2));
            nodesToVist.Push(rootNode);

            while (nodesToVist.Count > 0)
            {
                // Reset continuation value
                nodeVisitContext.Continuation = NodeIterationContinuation.ContinueAll;
                var visitedNode = nodesToVist.Pop();
                visitor(visitedNode, nodeVisitContext);
                if (nodeVisitContext.Continuation == NodeIterationContinuation.Stop)
                    return;

                var visitLeft = nodeVisitContext.Continuation == NodeIterationContinuation.ContinueAll || nodeVisitContext.Continuation == NodeIterationContinuation.ContinueLeftOnly;
                var visitRight = nodeVisitContext.Continuation == NodeIterationContinuation.ContinueAll || nodeVisitContext.Continuation == NodeIterationContinuation.ContinueRightOnly;
        ...
        /// <returns></returns>
        protected KdTreeNode FindNodeWithAggregator(KdTreeNode rootNode, Func<KdTreeNode, KdTreeNode, NodeVisitContext, KdTreeNode> aggregator)
        {
        ...
        static int PartialMedianSort(Span<BoundingBoxElement> elements, Axis currentAxis)
        {
        ...
        static int NthElement(Span<BoundingBoxElement> elements, int n, Axis currentAxis)
        {
            var startIndex = 0;
        ...
        static int QuickSelectPartition(Span<BoundingBoxElement> array, int startIndex, int endIndex, int pivotIndex, Axis currentAxis, out bool allEqual)
        {
