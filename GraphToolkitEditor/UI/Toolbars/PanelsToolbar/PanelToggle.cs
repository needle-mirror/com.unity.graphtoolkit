...
namespace Unity.GraphToolkit.Editor
{
    ...
    /// </summary>
    [UnityRestricted]
    internal abstract class PanelToggle : EditorToolbarToggle, IAccessContainerWindow
    {
        ...
        protected void OnAttachedToPanel(AttachToPanelEvent evt)
        {
            if (containerWindow.TryGetOverlay(WindowId, out m_OverlayWindow))
            {
                ...
                SetValueWithoutNotify(m_OverlayWindow.displayed);
            }
        }
        ...
        {
            // If displayed is changed to false we know the view will be destroyed so we inform the window.
            if (!displayed)
            {
                if (m_OverlayWindow is OverlayWithView overlayWithView && overlayWithView.rootView != null && containerWindow is GraphViewEditorWindow gvWindow)
                {
                    gvWindow.UnregisterView(overlayWithView.rootView);
                }
            }
